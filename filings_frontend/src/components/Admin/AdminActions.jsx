...
export const UsersActions = ({ params, rowId, setRowId }) => {
  ...
  const handleClickOpen = () => {
    ...
    if (endpoint) {
      fetch(`http://3.226.14.5:8000/api/v1/${endpoint}/${params.row.req_id}`)
        .then((res) => res.json())
        .then((result) => {
          setOutput(result);
        });
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm("Are you sure to delete this record?")) {
      const result = await fetch(
        "http://3.226.14.5:8000/api/v1/req-data-delete",
        {
          method: "DELETE",
  ...
  const handleSubmit = async () => {
    setLoading(true);
    const data = params.row;
    const result = await fetch("http://3.226.14.5:8000/api/v1/req-data-update", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
